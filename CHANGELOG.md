...
- **A push channel that stops delivering can no longer freeze the plugin.** A TV that answers the notification request without reporting anything kept the channel alive and looking healthy, so the failure count never built up and the fallback never kicked in. Such an answer no longer counts as a delivery, and independently of that, going a minute with no state update at all now brings interval polling back. HomeKit is never more than that behind the TV, whatever the channel does.
